...
- lint/typecheck/structure/build 全绿只证明工程门禁，不证明布局、交互或真实接口通过。按 TASKS DoD 和 A/I 逐项留证据；真实数据整页检查 1920/1366、深浅主题，平台未覆盖如实登记。
- PageCacheHost 缓存已挂载 React 树。验证前确认加载的是受检代码；记录提交及文件哈希。用真实菜单导航，不用 pushState+popstate 制造路由状态。
- IAB 可能冻结 rAF、ResizeObserver、关闭动画或返回旧合成帧。优先自然浏览器验证；诊断垫片仅用于隔离环境问题，不能代替真实用户交互验收，更不能进入产品代码。
- 必须使用 RO 垫片时，在 constructor 捕获回调并登记实例/目标；entry 同时提供 contentRect、borderBoxSize/contentBoxSize/devicePixelContentBoxSize。observe 后立即派发并持续有限重试至尺寸稳定，保留手动 fire 入口；需在组件 mount 前安装，已缓存页签不保证重新 mount。先用探针确认回调与非零尺寸，再断言表格数据。
- antd Modal/confirm 的关闭动画在 IAB 可能残留。不要 remove DOM 或写 display:none；先按业务取消关闭，必要时重新载入文档。确认操作须限定目标表格与最后一个有效确认框，以请求和真实状态核验，不凭残留 DOM 判定。
- antd v6 Select 打开定位 `.ant-select-content`；诊断性选中使用 pointerdown→mousedown→pointerup→mouseup→click。正式真实交互仍须实际指针验证。Upload 每次注入文件前重新查询 input，不能复用因 uploading 状态替换的旧节点。
- Apex 列设置是原生 dialog；开关和恢复默认只改草稿，点击面板内“确定(Enter)”才应用。用户保存偏好覆盖代码默认固定列；通过 data-pinned、计算样式和持久化切片交叉核验，清偏好后等待防抖完成再 reload。
- 主题为浅色/深色/跟随系统三态。定位顶栏用 lucide 结构类，不能依赖翻译 title。诊断性直写 settings 时值二次 JSON 序列化；此举不证明主题菜单可点。截图与 DOM 样式矛盾时重建合成帧后复核。
- 后端 root 为单会话；浏览器与 curl 重新登录会互踢，验证同一时段只使用一个登录会话。登录请求字段以真实 service 为准（username）；断言凭据失效前先核对字段与编码，不尝试猜口令。中文 curl 请求体使用 UTF-8 文件。
- 行内异步浮层使用页面级受控 open；禁用菜单项可能不显示 Tooltip，需要明确可达的说明。只验证合成事件或确认取消路径，不能报告命令执行成功。
- 锁恢复、检查点、Git 推送、最终验收与调度放行一律遵循 TASKS §2。自动轮先执行 prepare，让脚本绑定真实 session/turn、核验终态并对账远端；不能自拟会话 ID，不能凭 PID、mtime 或聊天中的“已完成”接管锁。每轮只处理一个任务，启动预检不占用额外业务轮次。
- 证据和无凭据验证脚本持久保存在 docs/migration/evidence；测试截图仅本地留存，由 .gitignore 排除，不提交 Git，不用强制添加、打包或内嵌图片绕过；文字记录、验证脚本和注明本地存储的索引可提交。不删唯一证据，不记录密码或完整 token。旧过程以对应任务交接记录为准，本节只保留当前方法，不继续叠加已被替代的试验过程。
