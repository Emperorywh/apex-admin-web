...
function defineAppRoutes<const T extends readonly AppRouteDefinition[]>(
	routes: T,
): T {
	return routes
}
...
export const appRouteDefinitions = defineAppRoutes([
	...
	},
	{
		...
		loadPage: () =>
			import('@/pages/action/ActionOrchestrationEdit/ActionOrchestrationEdit'),
	...
	},
	{
		...
		children: [
			...
			},
			{
				...
				path: 'robot-monitor',
				loadPage: () =>
					import('@/pages/robot-monitor/RobotMonitor/RobotMonitor'),
			...
			},
			{
				...
				children: [
					...
					},
					{
						...
						path: 'robots',
						loadPage: () =>
							import('@/pages/robot/RobotList/RobotList'),
					...
					},
					{
						...
						path: 'map-create',
						loadPage: () =>
							import('@/pages/robot/MapCreate/MapCreate'),
			...
			},
			{
				...
				loadPage: () =>
					import('@/pages/action/ActionOrchestration/ActionOrchestration'),
			...
			},
			{
				...
				loadPage: () =>
					import('@/pages/axis-motor/AxisMotorManagement/AxisMotorManagement'),
			...
			},
			{
				...
				loadPage: () =>
					import('@/pages/device/DeviceManagement/DeviceManagement'),
			...
			},
			{
				...
				loadPage: () =>
					import('@/pages/storage-location/StorageLocationManagement/StorageLocationManagement'),
			...
			},
			{
				...
				loadPage: () =>
					import('@/pages/task-chain/TaskChainOrchestration/TaskChainOrchestration'),
...
function collectRoutes(
	...
	for (const definition of definitions) {
		if (definition.id in ids)
			throw new Error(`路由 id 重复：${definition.id}`)
		ids[definition.id] = definition.id
		// index 与 * 节点没有可导航地址，归到父路径
		const navigable =
			definition.path !== undefined && !definition.path.includes('*')
		paths[definition.id] = navigable
			? joinPath(basePath, definition.path)
			: basePath
		if (definition.children?.length) {
			collectRoutes(
				definition.children,
				navigable ? paths[definition.id] : basePath,
			)
		}
	}
}
