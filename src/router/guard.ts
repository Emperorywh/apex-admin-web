/**
 * 认证与权限守卫（规格 §4.3，纯前端模式）：
 * - 每个受保护 loader 第一行 await rehydratedPromise，随后读取同一权限快照；
 ...
 * - /login loader 发现 token 时先 ensureProfile（本地快照），随后跳合法 redirect 或 /dashboard。
 */
...
/** 守卫依赖：测试注入桩实现隔离持久化；生产经 resolveDefaultGuardDeps 取默认单例 */
 ...
 * 无 token 时显示登录页；有 token 先 ensureProfile，随后跳合法 redirect 或 /dashboard
 * （redirect 值经五步同源校验，失败回 /dashboard）；token 在守卫执行期间被清理则继续显示登录页。
 */
