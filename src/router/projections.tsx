...
function getLazyPage(definition: AppRouteDefinition): ComponentType {
	...
	if (!definition.loadPage)
		throw new Error(`路由 ${definition.id} 缺少 loadPage`)
...
function toAccessNode(
	...
	if (isProtected) {
		if (
			...
			// 目标节点自带认证守卫，此处不再重复校验
			node.loader = () =>
				redirect(definition.redirect ?? ROOT_REDIRECT_TARGET)
		} else {
...
export const accessRoutes: RouteObject[] = appRouteDefinitions.map(
	(definition) =>
		// 登录页公开访问，其余顶层业务页面及其子路由保持认证保护。
		toAccessNode(
			definition,
			definition.id !== ROUTE_IDS['auth-login'],
			true,
		),
)
	...
	if (definition.loadPage) {
		...
		node.element = (
			<Suspense fallback={<PageLoading />}>
				<I18nPageGate
					page={LazyPage}
					namespaces={definition.meta.i18nNamespaces ?? []}
				/>
			</Suspense>
		)
	}
...
function filterMenuNodes(
	...
	for (const definition of definitions) {
		...
		if (definition.children?.length) {
			...
			!definition.index &&
			(definition.loadPage || definition.redirect)
		) {
			nodes.push({
				routeId: definition.id,
				path,
...
export function buildMenuRoutes(): MenuNode[] {
	...
	const rootIndex = tree.findIndex(
		(node) => node.routeId === ROUTE_IDS['root'],
	)
