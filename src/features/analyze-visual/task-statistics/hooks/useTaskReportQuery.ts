/**
 * 任务统计报表查询 hook（P35 私有；与 P33 useStatisticsQuery / P36
 * useAggregationQuery / P37 useVehicleStateQuery 同构的参数化查询，用户触发 +
 * 失败自动恢复——本页为报表页，不默认轮询，规格 8.2）。
 *
 * 本页三条查询链路（任务统计 / 利用率排行 / 利用率趋势）各自实例化本 hook，
 * 任一链路失败只降级对应区域，互不影响（旧实现同口径）。
 *
 * 行为契约（规格 8.2 / DoD 6 / D15，与 P33/P36/P37 同一份基线；契约变更须在
 ...
 *   不用旧数据或零值冒充成功（旧实现「失败仅留受控日志、保留上次数据降级」
 *   与现行 DoD 6 冲突，不迁移——差异登记 tasks/P35.md）；
 * - 失败自动恢复：KPI/图表是非表格数据块，按按钮纪律不设重试/刷新按钮，恢复依赖
...
export interface TaskReportQueryResult<P, T> {
...
export function useTaskReportQuery<P, T>(
  fetcher: (param: P, options?: RequestOptions) => Promise<T>,
): TaskReportQueryResult<P, T> {
    ...
    (param: P) => {
      ...
      void (async () => {
        try {
          ...
          // 真正失败：清空该区域远端数据（DoD 6），查询条件与草稿不受影响
          setData(null)
          setError(true)
