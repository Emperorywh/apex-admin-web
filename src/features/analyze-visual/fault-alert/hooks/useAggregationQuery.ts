/**
 * 告警聚合查询 hook（P36 私有；与 P33 useStatisticsQuery 同构的参数化查询，
 * 用户触发 + 失败自动恢复，非持续轮询页——本页为报表页，不默认轮询）。
 *
 * 行为契约（规格 8.2 / DoD 6 / D15，与 P33 同一份基线）：
 ...
 *   不用旧数据或零值冒充成功（TASKS P36 专项：「查询失败不能显示无告警」）；
 * - 失败自动恢复：聚合区是非表格数据块，按按钮纪律不设重试/刷新按钮，恢复依赖
 *   可见状态下的有限退避自动重查（上限 3 次，页签非激活或文档不可见时不发请求），
 *   以及用户点「查询」；成功后重查计数归零。
 *
 * 与 P33 useStatisticsQuery 分文件说明：两页各自持有私有 hook（项目现状
 * P33/P34 亦各自实现查询链路），避免跨页文件耦合；行为契约同源，
 * 契约变更须同步两处（contracts.md 报表查询链路节登记）。
 */
...
export interface AggregationQueryResult<P, T> {
...
export function useAggregationQuery<P, T>(
  fetcher: (param: P, options?: RequestOptions) => Promise<T>,
): AggregationQueryResult<P, T> {
    ...
    (param: P) => {
      ...
      void (async () => {
        try {
          ...
          // 真正失败：清空聚合区域远端数据（DoD 6），筛选条件与草稿不受影响
          setData(null)
          setError(true)
