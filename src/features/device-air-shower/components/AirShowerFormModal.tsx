/**
 * 新增/编辑风淋门弹窗（P18）：真实 addAirShowerDoor/updateAirShowerDoor 接入。
 *
 * 交互与契约要点（与 P15 自动门表单同构，差异按旧实现逐项保留）：
 ...
 *   （P15 起定版；旧版 allowClear 反选清空配置易误触丢稿，不再沿用）；
 * - 设备配置校验：必填 + JSON.parse 真实解析（解析失败报错并保留输入，
 *   不做半吊子格式提示）；提交为解析后的对象原样上送；
 * - 与自动门表单的字段差异（旧实现如此，OpenAPI AirShowerDoorParam 亦无）：
 *   无「是否启用」字段——提交体不含 deviceStatus（不发明 UI）；
 *   驱动字段 label「设备驱动」、ip 字段 label「ip地址」（小写）均为旧版原样；
 * - 数值边界（旧实现同边界）：端口 0..65535 整数；
...
interface AirShowerFormModalProps {
  open: boolean
  /** 编辑目标行；null = 新增模式 */
  editTarget: AirShowerDoorDto | null
  /** 驱动集合（页面加载；为空时驱动下拉展示加载失败/空态，不阻塞弹窗打开） */
  drivers: AirShowerDoorDriverDto[]
  onClose: () => void
  /** 提交成功后回调（页面刷新列表；本组件已自行复位表单） */
  onSucceeded: () => void
}

export function AirShowerFormModal({
  open,
  editTarget,
  drivers,
  onClose,
  onSucceeded,
}: AirShowerFormModalProps) {
  const { t } = useTranslation('deviceAirShower')
  const { message } = App.useApp()
  const [form] = Form.useForm<AirShowerFormValues>()
  const [submitting, setSubmitting] = useState(false)
  const isEdit = editTarget !== null

  // 草稿脏标记：任何输入即登记到页签（关页/刷新统一确认、LRU 豁免）；编辑模式同样保护
  const [dirty, setDirty] = useState(false)
  useTabDirtyGuard(dirty, isEdit ? t('编辑风淋门') : t('新增风淋门'))
  ...
  const handleOk = async () => {
    ...
    try {
      if (isEdit && editTarget) {
        ...
        // 新增：不传 deviceKey（后端生成，旧实现同边界）
        await addAirShowerDoor({
          deviceName: values.deviceName.trim(),
          driverKey: values.driverKey,
          ip: values.ip.trim(),
          port: values.port,
      ...
      if (!isCancelledError(error)) {
        message.error((isEdit ? t('编辑风淋门出错') : t('新增风淋门出错')) + apiErrorMessage(error))
      }
    } finally {
      setSubmitting(false)
    }
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          {/* label 小写「ip地址」为旧版表单原样（列表列头为「IP地址」） */}
