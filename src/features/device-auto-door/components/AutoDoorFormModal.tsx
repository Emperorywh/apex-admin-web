/**
 * 新增/编辑自动门弹窗（P15）：真实 addAutoDoor/updateAutoDoor 接入。
 *
 * 交互与契约要点（与 P14 电梯表单同构，差异按旧实现逐项保留）：
 ...
 * - 与电梯表单的字段差异（旧实现如此）：无楼层字段（自动门无楼层语义）；
...
interface AutoDoorFormModalProps {
  open: boolean
  /** 编辑目标行；null = 新增模式 */
  editTarget: AutoDoorDto | null
  /** 驱动集合（页面加载；为空时驱动下拉展示加载失败/空态，不阻塞弹窗打开） */
  drivers: AutoDoorDriverDto[]
  onClose: () => void
  /** 提交成功后回调（页面刷新列表；本组件已自行复位表单） */
  onSucceeded: () => void
}

export function AutoDoorFormModal({
  open,
  editTarget,
  drivers,
  onClose,
  onSucceeded,
}: AutoDoorFormModalProps) {
  const { t } = useTranslation('deviceAutoDoor')
  const { message } = App.useApp()
  const [form] = Form.useForm<AutoDoorFormValues>()
  const [submitting, setSubmitting] = useState(false)
  const isEdit = editTarget !== null

  // 草稿脏标记：任何输入即登记到页签（关页/刷新统一确认、LRU 豁免）；编辑模式同样保护
  const [dirty, setDirty] = useState(false)
  useTabDirtyGuard(dirty, isEdit ? t('编辑自动门') : t('添加自动门'))
  ...
  const handleOk = async () => {
    ...
    try {
      ...
      if (!isCancelledError(error)) {
        message.error((isEdit ? t('更新自动门出错') : t('添加自动门出错')) + apiErrorMessage(error))
      }
    } finally {
      setSubmitting(false)
    }
