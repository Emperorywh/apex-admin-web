...
export function WorkflowPicker({
		...
		const matched = ACTION_PRESETS.filter((preset) => {
			const belongsToSection =
				section === 'action'
					...
					: preset.kind !== 'action'
			const searchable =
				`${preset.label} ${preset.group} ${t(preset.label)} ${t(preset.group)}`.toLocaleLowerCase()
	...
	return (
		<div
			...
			style={
				{
					'--picker-x': `${position.x}px`,
					'--picker-y': `${position.y}px`,
				} as CSSProperties
			}
		...
		>
			...
			<div className={styles.search}>
				<Input
					...
					placeholder={t('搜索节点名称或动作库')}
					aria-label={t('搜索节点名称或动作库')}
					allowClear
				/>
			</div>
			<div
			...
			>
				<button
					...
					type="button"
					role="tab"
					aria-selected={section === 'action'}
					aria-controls={`${dialogId}-content`}
					className={section === 'action' ? styles.activeTab : ''}
				...
				<button
					...
					type="button"
					role="tab"
					aria-selected={section === 'logic'}
					aria-controls={`${dialogId}-content`}
					className={section === 'logic' ? styles.activeTab : ''}
					onClick={() => setSection('logic')}
				>
					<Workflow size={14} />
					{t('逻辑节点')}
				</button>
			...
			<div
				...
				role="tabpanel"
				aria-labelledby={`${dialogId}-${section}`}
			>
				{groups.length === 0 && (
					<Empty
				...
				{groups.map(([group, presets], index) => {
					const expanded =
						Boolean(query.trim()) || !collapsedGroups.has(group)
					return (
						<section key={group} className={styles.group}>
							<button
								...
								aria-controls={`${dialogId}-group-${index}`}
								onClick={() => toggleGroup(group)}
							>
								<ChevronDown
									size={13}
									className={
										expanded ? '' : styles.collapsedChevron
									}
								/>
								<span>{t(group)}</span>
