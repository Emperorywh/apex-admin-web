...
export const WorkflowEdge = memo(function WorkflowEdge({ id, source, sourceHandleId, sourceX, sourceY, sourcePosition, targetX, targetY, targetPosition, selected, markerStart, markerEnd, style }: EdgeProps<WorkflowEdgeData>) {
  ...
  return <>
    <g onMouseEnter={() => setHovered(true)} onMouseLeave={() => setHovered(false)}>
      <BaseEdge id={id} path={path} markerStart={markerStart} markerEnd={markerEnd} interactionWidth={22} style={{ ...style, stroke: selected ? 'var(--app-cyan)' : 'var(--app-blue)', strokeWidth: selected ? 2 : 1.5 }} />
    </g>
    <EdgeLabelRenderer>
      <button
        type="button"
        ...
        onClick={(event) => { event.stopPropagation(); actions.onAdd(source, sourceHandleId ?? 'output') }}
