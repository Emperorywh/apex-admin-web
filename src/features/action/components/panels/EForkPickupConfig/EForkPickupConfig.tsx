...
export function EForkPickupConfig({
	nodeId,
	data,
	motors,
	onApply,
	onPendingChange,
}: EForkPickupConfigProps) {
	...
	const parametersDirty =
		draft.liftHeight !== applied.liftHeight ||
		draft.encoderValue !== applied.encoderValue
	...
	const applyBinding = () => {
		...
		if (
			...
			void message.success(t('轴电机绑定已应用到当前节点'))
	}
	...
	const applyParameters = () => {
		...
		if (errors.length) {
			void message.error(t(errors[0]))
			return
		}
		if (onApply({ pickup: { ...draft } }))
			void message.success(t('动作参数已应用到当前节点'))
	}
		...
		<div className={styles.config}>
			<div className={styles.field}>
				<label htmlFor={fieldId('code')}>
					{t('动作所属编号')}
					<span>{t('由动作类型自动确定')}</span>
				</label>
				<Input
					id={fieldId('code')}
					value={EFORK_PICKUP_CODE}
					readOnly
				/>
			</div>
			<div className={styles.field}>
				<label htmlFor={fieldId('motor')}>{t('动作轴绑定')}</label>
				<Select
					id={fieldId('motor')}
					value={motor?.id}
					placeholder={t('选择顶升轴电机')}
					showSearch={{ optionFilterProp: 'label' }}
					options={options.map((item) => ({
						value: item.id,
						label: `${t(item.name)} · ${t('轴 {{number}}', { number: getMotorAction(item.actionType).axisNumber })}`,
					}))}
					notFoundContent={t(
						'当前车体暂无可用顶升轴，请先配置轴电机',
					)}
			...
			{!!applied.motorId &&
				!options.some((item) => item.id === applied.motorId) && (
			...
			<div className={styles.parameters}>
				<h4>
					...
					{motor && (
						<span>
							{t('轴 {{number}}', {
								number: getMotorAction(motor.actionType)
									.axisNumber,
							})}
						</span>
					)}
				...
				<div className={styles.field}>
					...
					<InputNumber
						id={fieldId('height')}
						disabled={!motor || bindingDirty}
						value={draft.liftHeight}
						min={motor?.parameters.lowerPosition}
						max={motor?.parameters.upperPosition}
						precision={2}
						placeholder={t('输入顶升高度')}
						onChange={(liftHeight) =>
							setDraft({
								...draft,
								liftHeight,
								encoderValue:
									motor && liftHeight !== null
										? heightToEncoder(motor, liftHeight)
										: null,
							})
						}
					/>
				...
				<div className={styles.field}>
					<label htmlFor={fieldId('encoder')}>{t('编码器值')}</label>
					<InputNumber
						id={fieldId('encoder')}
						disabled={!motor || bindingDirty}
						value={draft.encoderValue}
						min={motor?.parameters.lowerEncoder}
						max={motor?.parameters.upperEncoder}
						precision={0}
						placeholder={t('输入编码器值')}
						onChange={(encoderValue) =>
							setDraft({
								...draft,
								encoderValue,
								liftHeight:
									motor && encoderValue !== null
										? encoderToHeight(motor, encoderValue)
										: null,
							})
						}
					/>
				...
				<Button
					block
					icon={<Check size={13} />}
					disabled={!motor || bindingDirty || !parametersDirty}
			...
			<div className={styles.enable}>
				...
				<Switch
					...
					disabled={bindingDirty || parametersDirty}
					checkedChildren={t('启用')}
					unCheckedChildren={t('停用')}
					onChange={(enabled) =>
						onApply({ pickup: { ...applied, enabled } })
					}
				/>
			</div>
			{(bindingDirty || parametersDirty) && (
				<p className={styles.hint}>
					{t('有未应用的配置，切换节点将放弃这些修改。')}
				</p>
			)}
			<WorkflowNodeTest
				key={JSON.stringify([
				...
				data={data}
				motors={motors}
				pending={bindingDirty || parametersDirty}
			/>
		</div>
	)
}
