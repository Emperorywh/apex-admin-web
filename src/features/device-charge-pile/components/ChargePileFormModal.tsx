/**
 * 新增/编辑充电桩弹窗（P16）：真实 addChargePile/updateChargePile 接入。
 *
 * 交互与契约要点（与 P14/P15 电梯/自动门表单同构，差异按旧实现逐项保留）：
 ...
 * - 驱动切换（任务卡「驱动参数」核验点）：选中驱动即以其 driverProtocol
 *   覆写设备配置 JSON（旧实现同语义）；不提供清除选择入口，保留用户已编辑值；
 * - 设备配置校验：必填 + JSON.parse 真实解析（解析失败报错并保留输入，
 *   不做半吊子格式提示）；提交为解析后的对象原样上送；
 * - 与电梯/自动门表单的字段差异（旧版表单原样）：ip 字段 label「设备IP地址」、
 *   port 字段 label「设备端口号」（全称，非自动门的小写形态）；ip 增加格式
 *   校验（旧实现同规则：IPv4 正则，「请输入正确的IP地址」）；无楼层/网络字段；
 * - 数值边界（旧实现同边界）：端口 0..65536 整数（旧版 max=65536 原样迁移，
 *   较通用 TCP 上界 65535 宽 1，登记 P16.md 交接记录）；
...
/**
 * IPv4 格式正则（旧实现同规则原样迁移：四段 0-255，允许首尾空白）；
 * 仅本表单消费，不外提公共常量（T00 公共文件不动）。
 */
const IP_REGEXP = /^\s*((25[0-5]|2[0-4]\d|[01]?\d\d?)\.){3}(25[0-5]|2[0-4]\d|[01]?\d\d?)\s*$/

/** 表单值形状（设备配置在表单层为 JSON 文本，提交时解析为对象） */
interface ChargePileFormValues {
  deviceName: string
  driverKey: string
  ip: string
  port: number
...
interface ChargePileFormModalProps {
  open: boolean
  /** 编辑目标行；null = 新增模式 */
  editTarget: ChargePileDto | null
  /** 驱动集合（页面加载；为空时驱动下拉展示空态，不阻塞弹窗打开） */
  drivers: ChargePileDriverDto[]
  onClose: () => void
  /** 提交成功后回调（页面刷新列表；本组件已自行复位表单） */
  onSucceeded: () => void
}

export function ChargePileFormModal({
  open,
  editTarget,
  drivers,
  onClose,
  onSucceeded,
}: ChargePileFormModalProps) {
  const { t } = useTranslation('deviceChargePile')
  const { message } = App.useApp()
  const [form] = Form.useForm<ChargePileFormValues>()
  const [submitting, setSubmitting] = useState(false)
  const isEdit = editTarget !== null

  // 草稿脏标记：任何输入即登记到页签（关页/刷新统一确认、LRU 豁免）；编辑模式同样保护
  const [dirty, setDirty] = useState(false)
  useTabDirtyGuard(dirty, isEdit ? t('编辑充电桩') : t('新增充电桩'))
  ...
  const handleOk = async () => {
    ...
    try {
      ...
      if (!isCancelledError(error)) {
        message.error((isEdit ? t('编辑充电桩出错') : t('新增充电桩出错')) + apiErrorMessage(error))
      }
    } finally {
      setSubmitting(false)
    }
  ...
  /** ip 字段规则：必填 + IPv4 格式（旧实现同规则，validateFirst 同旧形态） */
  const ipRules = [
    ...
    >
      ...
      >
        ...
        <Form.Item
          ...
          rules={ipRules}
        >
          {/* label「设备IP地址」为旧版表单原样；IPv4 格式校验失败不提交 */}
          <Input maxLength={64} showCount placeholder={t('请输入设备IP地址')} />
        </Form.Item>
        ...
        >
          {/* 端口整数语义：0..65536（旧版 max=65536 原样迁移，登记交接记录） */}
          <InputNumber
            style={{ width: '100%' }}
