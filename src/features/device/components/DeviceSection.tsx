...
export function DeviceSection<T extends DeviceRecord>({
	title,
	deviceLabel,
	initialRecords,
	defaults,
	...
	scrollWidth,
	getName,
}: DeviceSectionProps<T>) {
	const { message } = App.useApp()
	const [records, setRecords] = useState<T[]>(() =>
		structuredClone(initialRecords),
	)
			...
			render: (_value, record) => (
				<Space size={12}>
					<Button
						...
						type="link"
						size="small"
						aria-label={`修改${getName(record)}`}
						onClick={() => setEditor({ record })}
					>
						修改
					</Button>
					<Popconfirm
						title={`删除${deviceLabel}`}
						description={`确定删除“${getName(record)}”吗？`}
						okText="删除"
						cancelText="取消"
	...
	return (
		<section className={styles.section} aria-label={title}>
				...
				locale={{
					emptyText: (
						<Empty
							...
							description={`暂无${deviceLabel}，点击右上角新增`}
						/>
					),
				}}
			/>
			...
			{editor && (
				<DeviceEditor<T>
					title={`${editor.record ? '修改' : '新增'}${deviceLabel}`}
					initialValues={editor.record ?? defaults}
					onSave={saveRecord}
...
function DeviceEditor<T extends DeviceRecord>({
	title,
	initialValues,
	onSave,
	onCancel,
