...
export function MotorAttributesFields({ form }: MotorAttributesFieldsProps) {
	const actionType = Form.useWatch<MotorAttributes['actionType']>(
		'actionType',
		form,
	)
		...
		<Row gutter={16}>
			...
			<Col xs={24} sm={12}>
				<Form.Item
					name="actionType"
					label="动作类型"
					tooltip="更换动作类型并保存后，控制与标定参数将按新动作重新初始化。"
					rules={[{ required: true, message: '请选择动作类型' }]}
				>
					<Select
						placeholder="请选择动作类型"
						options={axisMotorData.actions.map((item) => ({
							value: item.id,
							label: item.label,
						}))}
					/>
			...
			<Col xs={24} sm={12}>
				<Form.Item
					label="轴所属编号"
					tooltip="根据所选动作类型自动关联，无需手动输入。"
				>
					<Input
						aria-label="轴所属编号"
						value={action?.axisNumber ?? ''}
						readOnly
						placeholder="选择动作后自动显示"
					/>
				</Form.Item>
			</Col>
			<Col xs={24} sm={12}>
				<Form.Item
					name="driverId"
					label="轴驱动器"
					rules={[{ required: true, message: '请选择轴驱动器' }]}
				>
					<Select
						placeholder="请选择轴驱动器"
						options={axisMotorData.drivers}
					/>
				</Form.Item>
			</Col>
			<Col xs={24} sm={12}>
				<Form.Item
					name="encoderId"
					label="轴编码器"
					rules={[{ required: true, message: '请选择轴编码器' }]}
				>
					<Select
						placeholder="请选择轴编码器"
						options={axisMotorData.encoders}
					/>
				</Form.Item>
			</Col>
		</Row>
