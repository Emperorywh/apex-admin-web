...
import type {
	...
	MotorAttributes,
	MotorParameters,
} from '@/features/axis-motor/axisMotor.types'
...
export function MotorCard({
	...
	const toEncoder = (position: number) =>
		Math.round(
			lowerEncoder +
				((position - lowerPosition) / (upperPosition - lowerPosition)) *
					(upperEncoder - lowerEncoder),
		)
	const positiveEncoder = action.hasTargetEncoder
		? targetEncoder!
		: toEncoder(positiveTarget)
	...
	useEffect(() => {
		...
		if (
			previous.name === motor.name &&
			previous.actionType === motor.actionType &&
			previous.driverId === motor.driverId &&
			previous.encoderId === motor.encoderId
		)
			return
		syncedAttributes.current = {
			name: motor.name,
			actionType: motor.actionType,
			driverId: motor.driverId,
			encoderId: motor.encoderId,
		}
	...
	useEffect(() => {
		...
		const timer = window.setInterval(() => {
			...
			const next =
				direction === 1
					? Math.min(target, encoderRef.current + step)
					: Math.max(target, encoderRef.current - step)
	...
	return (
		<Card
			...
			title={
				<Space size={8} wrap>
					<Typography.Text strong>{motor.name}</Typography.Text>
					<Tag color="cyan">轴所属编号：{action.axisNumber}</Tag>
				</Space>
			}
			extra={
				<Badge
					status={direction ? 'processing' : 'default'}
					text={
						direction
							? `${direction === 1 ? action.positiveLabel : action.negativeLabel}中`
							: '已停止'
					}
				/>
			}
		>
			...
			>
				<span className={styles.parametersToggleLabel}>
					<ChevronRight size={14} aria-hidden="true" />
					轴电机控制参数
				</span>
				<span className={styles.hint}>
					{parametersExpanded ? '收起' : '展开'}
				</span>
			</button>
			<div
				id={parametersPanelId}
				hidden={!parametersExpanded}
				...
				>
								...
								rules={[
									...
									({ getFieldValue }) => ({
										validator: (_, value) => {
											...
											if (
												value == null ||
												action.hasTargetEncoder
											)
												return Promise.resolve()
											const lower =
												getFieldValue('lowerEncoder')
											const scale =
												(getFieldValue('upperEncoder') -
													lower) /
												(getFieldValue(
													'upperPosition',
												) -
					...
					<Row gutter={12}>
						<Col span={12}>
							<Form.Item
								name="upperPosition"
								label={`${action.positionLabel}上限`}
								dependencies={['lowerPosition']}
								rules={[
									{
										required: true,
										message: '请输入位置上限',
					...
					>
						<Space size={8}>
							...
							<Typography.Text className={styles.encoderValue}>
								{encoder.toLocaleString()}
							</Typography.Text>
							<Tag>模拟</Tag>
						</Space>
						{action.hasLimitSwitches && (
							<Space size={12} wrap>
								<Badge
									status={
										atUpperLimit ? 'warning' : 'default'
									}
									text={`上限位 IO · ${atUpperLimit ? '已触发' : '未触发'}`}
								/>
								<Badge
									status={
										atLowerLimit ? 'warning' : 'default'
									}
									text={`下限位 IO · ${atLowerLimit ? '已触发' : '未触发'}`}
								/>
							</Space>
						)}
					</Flex>
					...
					<Flex className={styles.testActions} gap={8} wrap>
						...
						<Button
							...
							disabled={
								...
								attributesDirty ||
								parametersDirty ||
								encoder <= negativeEncoder
							}
							onClick={() => startTest(-1)}
						>
							{action.negativeLabel}测试
						</Button>
						<Button
							size="small"
							danger
					...
					{parametersDirty && (
						<Typography.Text type="warning" className={styles.hint}>
							参数有未保存的修改，保存后可测试
						</Typography.Text>
					)}
				</Form>
			...
			<Button
				...
				size="small"
				block
				onClick={() => onAdd(motor.actionType)}
			>
				新增轴电机
			</Button>
		</Card>
