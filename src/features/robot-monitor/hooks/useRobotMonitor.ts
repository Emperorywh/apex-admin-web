...
function withEvent(
	...
	return {
		...state,
		events: [
			{ id: (state.events[0]?.id ?? 0) + 1, time, message, tone },
			...state.events,
		].slice(0, 30),
	}
}
...
function monitorReducer(
	...
	switch (action.type) {
		case 'tick': {
			...
			if (state.mode !== 'running' && state.mode !== 'returning')
				return state
			...
			const distance = Math.max(
				...
				Math.min(
					TOTAL_DISTANCE,
					state.distance + direction * ROUTE_SPEED * seconds,
				),
			)
			const actualSeconds =
				Math.abs(distance - state.distance) / ROUTE_SPEED
			const nextState = {
				...state,
				distance,
		...
		case 'resume':
			...
			if (state.travelMode === 'returning' && state.distance === 0) {
				return withEvent(
					{ ...state, mode: 'charging' },
					action.time,
					'机器人已位于充电桩 P01，开始自动充电',
					'success',
				)
			}
			return withEvent(
				{ ...state, mode: state.travelMode },
				action.time,
				state.travelMode === 'returning'
					? '已继续返航充电任务'
					: '已继续配送任务',
				'success',
			)
		...
		case 'returnToCharge':
			...
			if (state.distance === 0) {
				return withEvent(
					{ ...state, mode: 'charging', travelMode: 'returning' },
					action.time,
					'机器人已位于充电桩 P01，开始自动充电',
					'success',
				)
			...
			return withEvent(
				{ ...state, mode: 'returning', travelMode: 'returning' },
				action.time,
				'返航指令已确认，沿原路线返回充电桩 P01',
				'info',
			)
		case 'relocate': {
			if (
			...
			const station = STATIONS.find(
				(item) => item.id === action.stationId,
			)
			if (!station) return state
			return withEvent(
				{ ...state, distance: station.distance, mode: 'paused' },
				action.time,
				`已重定位至 ${station.label} ${station.id}，等待手动继续`,
				'info',
			)
		}
		case 'restart':
			if (
				(state.mode !== 'completed' && state.mode !== 'charging') ||
				state.battery <= 0
			)
				return state
			return withEvent(
...
export function useRobotMonitor() {
	...
	const isAnimating =
		state.mode === 'running' ||
		state.mode === 'returning' ||
		(state.mode === 'charging' && state.battery < 100)
	...
	const pause = useCallback(
		() => dispatch({ type: 'pause', time: eventTime() }),
		[],
	)
	const resume = useCallback(
		() => dispatch({ type: 'resume', time: eventTime() }),
		[],
	)
	const emergencyStop = useCallback(
		() => dispatch({ type: 'emergencyStop', time: eventTime() }),
		[],
	)
	const resetEmergency = useCallback(
		() => dispatch({ type: 'resetEmergency', time: eventTime() }),
		[],
	)
	const returnToCharge = useCallback(
		() => dispatch({ type: 'returnToCharge', time: eventTime() }),
		[],
	)
	const relocate = useCallback(
		(stationId: string) =>
			dispatch({ type: 'relocate', stationId, time: eventTime() }),
		[],
	)
	const restart = useCallback(
		() => dispatch({ type: 'restart', time: eventTime() }),
		[],
	)
	...
	const position = {
		...routePosition,
		heading:
			routePosition.heading +
			(state.travelMode === 'returning' ? 180 : 0),
	}
