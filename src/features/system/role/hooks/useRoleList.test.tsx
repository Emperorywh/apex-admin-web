...
const TEST_SCOPE_ID = 'role-list-test-tab'

function roleFixture(id: string, code: string): Role {
  return {
    id,
    code,
    name: `${code} 名称`,
...
function pageFixture(codes: string[]): PageResult<Role> {
  return {
    list: codes.map((code, index) => roleFixture(`r-${index + 1}`, code)),
    total: codes.length,
    page: 1,
    size: 10,
  }
...
describe('useRoleList（规格 §14.3/§17.24）', () => {
  it('挂载即加载：GET role 域 endpoint、默认第 1 页/size 10、未传 sortBy、附页签作用域', async () => {
  ...
  it('changeSort：发送白名单 sortBy/sortOrder 并重置回第 1 页；undefined 恢复默认排序', async () => {
    ...
    await act(async () => {
      result.current.changeSort('code', 'desc')
    })
    await waitFor(() =>
      expect(capturedConfig(2).params).toMatchObject({ sortBy: 'code', sortOrder: 'desc', page: 1 }),
    )
    expect(result.current.query.sortBy).toBe('code')
  ...
  it('§17.24：快速连续查询时前请求的调用方 signal 被 abort，后请求结果胜出', async () => {
    ...
    requestSpy.mockImplementationOnce(
      () =>
        new Promise<PageResult<Role>>((resolve) => {
    ...
    expect(result.current.roles.map((role) => role.code)).toEqual(['fresh'])
  })

  it('§17.24：乱序返回的陈旧响应（未被取消）不覆盖后请求结果', async () => {
    let resolveFirst!: (page: PageResult<Role>) => void
    requestSpy.mockImplementationOnce(
      () =>
        new Promise<PageResult<Role>>((resolve) => {
    ...
    await waitFor(() => expect(result.current.roles.map((role) => role.code)).toEqual(['second']))

    // 第一次（陈旧）请求此时才落定：不得覆盖第二次结果
    await act(async () => {
      resolveFirst(pageFixture(['stale-first']))
    })
    expect(result.current.roles.map((role) => role.code)).toEqual(['second'])
  })
  ...
  it('§17.24：被取消的请求以拒绝落定时同样静默，不覆盖后请求结果', async () => {
    ...
    requestSpy.mockImplementationOnce(
      () =>
        new Promise<PageResult<Role>>((_resolve, reject) => {
    ...
    await waitFor(() => expect(result.current.roles.map((role) => role.code)).toEqual(['fresh']))
