...
import {
	buildWorkflowEditorTheme,
	workflowEditorVariables,
} from './actionOrchestrationEdit.theme'
...
function WorkflowSession({ id }: { id: string }) {
	const [initial] = useState(() =>
		createWorkflowDocument(
			id,
			initialCombinations.find((item) => item.id === id),
		),
	)
	return (
		<ReactFlowProvider>
			<WorkflowEditor initial={initial} />
		</ReactFlowProvider>
	)
}
...
function WorkflowEditor({ initial }: { initial: WorkflowDocument }) {
	...
	const {
		...
		canUndo,
		canRedo,
	} = useWorkflowHistory(initial)
	// 轴配置与电机页面读取同一份当前数据；读取异常时不允许用默认值验证实际绑定。
	const { motors: configuredMotors, storageWarning: motorStorageWarning } =
		useAxisMotors()
	const motors = useMemo(
		() => (motorStorageWarning ? [] : configuredMotors),
		[configuredMotors, motorStorageWarning],
	)
	...
	const issues = useMemo(
		() => validateWorkflow(workflow, motors),
		[workflow, motors],
	)
	...
	const updateData = (data: Partial<WorkflowNodeData>) =>
		change((current) => {
			...
			const edges = data.branches
				? current.edges.filter(
						(edge) =>
							edge.source !== selectedId ||
							edge.sourceHandle === 'else' ||
							data.branches!.some(
								(branch) => branch.id === edge.sourceHandle,
							),
					)
				: current.edges
	...
	const duplicateNode = useCallback(
		(nodeId: string) => {
			const source = latest.current.nodes.find(
				(node) => node.id === nodeId,
			)
	...
	const openPicker = useCallback(
		(position?: XYPosition, source?: string, handle = 'output') => {
			const rect = canvasRef.current?.getBoundingClientRect()
			if (!rect) return
			const point = position ?? {
				x: Math.min(rect.width / 2, 520),
				y: 90,
			}
	...
	const addNext = useCallback(
		(nodeId: string, handle: string) =>
			openPicker(undefined, nodeId, handle),
		[openPicker],
	)
	...
	const addNode = (preset: ActionPreset) => {
		if (!picker) return
		const previous = workflow.edges.find(
			(edge) =>
				edge.source === picker.source &&
				edge.sourceHandle === picker.handle,
		)
		if (previous && preset.kind === 'end') {
			void message.warning(
				t('已连接的路径不能插入结束节点，请从空闲出口添加'),
			)
			return
		}
			...
			return {
				...
				nodes: [
					...current.nodes.map((item) => ({
						...
						selected: false,
						position:
							previous && item.position.x >= node.position.x
								? { ...item.position, x: item.position.x + 330 }
								: item.position,
					})),
					node,
				],
	...
	const fit = useCallback(() => {
		void flow.fitView({ padding: 0.22, maxZoom: 1, duration: 260 })
	}, [flow])
	...
	useEffect(() => {
		...
		if (
			...
			point.x + 236 * currentZoom > rect.right - 30 ||
			point.y < rect.top + 50 ||
			point.y + nodeHeight * currentZoom > rect.bottom - 80
		) {
			void flow.setCenter(
				node.position.x + 118,
				node.position.y + nodeHeight / 2,
				{ zoom: currentZoom, duration: 240 },
			)
		}
	}, [selectedId, flow])
	...
	const exportDocument = () => {
		...
		anchor.href = url
		anchor.download = `${workflow.name.replace(/[<>:"/\\|?*]/g, '_') || 'workflow'}.json`
		anchor.click()
	...
	const saveDocument = useCallback(() => {
		...
		void message.info(
			t('流程检查通过，保存接口暂未接入，可先导出流程 JSON 保留文件'),
		)
	...
	useEffect(() => {
		const onKeyDown = (event: KeyboardEvent) => {
			...
			else if (event.key === 'Tab' && target.closest('.react-flow')) {
				event.preventDefault()
				openPicker()
			}
		}
	...
	const actions = useMemo(
		...
		[addNext, duplicateNode, deleteNode],
	)
	...
	const toolButton = (
		...
		<Tooltip
			title={t(label)}
			placement={placement}
			classNames={{ root: styles.toolTooltip }}
		>
			<Button
				type="text"
	...
	return (
		<ConfigProvider
			theme={editorTheme}
			modal={{ styles: { root: workflowEditorVariables } }}
		>
			<main className={styles.editor} style={workflowEditorVariables}>
				...
				<header className={styles.header}>
					<Button
						...
						onClick={() =>
							void navigate(ROUTE_PATHS['action-orchestration'])
						}
					/>
					...
					<div className={styles.documentInfo}>
						...
						<div className={styles.editState}>
							<span
								className={
									dirty ? styles.warningDot : styles.idleDot
								}
							/>
							{dirty ? t('有未保存的修改') : t('尚未修改')}
							<span>·</span>
							{t('离开后不保留')}
						</div>
					</div>
					<div className={styles.headerCenter}>
						<span className={styles.headerTab}>{t('编排')}</span>
					</div>
					<div className={styles.headerActions}>
						...
						<Tooltip title={t('导出流程 JSON')}>
							<Button
								className={styles.exportButton}
								aria-label={t('导出流程 JSON')}
				...
				<div className={styles.workspace}>
					<section
						...
						onKeyDownCapture={(event) => {
							...
							if (
								...
								workflow.nodes.some((node) => node.selected)
							)
								checkpoint()
						}}
					>
						<WorkflowNodeActions.Provider value={actions}>
							...
							<ReactFlow<WorkflowNode, WorkflowEdge>
								...
								isValidConnection={(connection) =>
									canConnect(workflow.nodes, workflow.edges, {
										...connection,
										sourceHandle:
							...
							>
								...
								{showMap && (
									<MiniMap
										...
										pannable
										zoomable
										nodeColor="var(--app-track)"
										nodeStrokeColor="var(--app-blue)"
										maskColor="var(--app-minimap-mask)"
									/>
								)}
							</ReactFlow>
						...
						<button
							...
							onClick={() => setVariables(true)}
						>
							<Braces size={14} />
							{t('变量检查')}
						</button>
						<div className={styles.zoomControls}>
							{toolButton('缩小', <Minus size={15} />, () => {
					...
					{selected && (
						<WorkflowInspector
							...
							onPendingChange={setPendingNodeChanges}
							onChange={updateData}
							onClose={() => selectNode(null)}
							onDelete={() => deleteNode(selected.id)}
							onDuplicate={() => duplicateNode(selected.id)}
							onAddNext={(handle) => addNext(selected.id, handle)}
				...
				>
					...
					<div className={styles.variableList}>
						...
						{!start?.data.inputs.length && (
							<p>{t('暂无输入变量，可在开始节点中添加')}</p>
						)}
						{workflow.nodes
							.filter(
								(node) =>
