...
import {
	...
	robotTypeOptions,
	sceneOptions,
} from '@/features/action/action.model'
...
export default function ActionOrchestration() {
	...
	const openEditor = (id?: string) => {
		void navigate({
			pathname: ROUTE_PATHS['action-orchestration-edit'],
			search: id ? `?${new URLSearchParams({ id })}` : '',
		})
	}
		...
		(record) =>
			...
			(!query.updatedRange ||
				(dayjs(record.updatedAt).valueOf() >=
					query.updatedRange[0].startOf('day').valueOf() &&
					dayjs(record.updatedAt).valueOf() <=
						query.updatedRange[1].endOf('day').valueOf())),
	)
	...
	const changeStatus = (record: ActionCombination) =>
		modal.confirm({
			title: record.enabled ? t('停用动作组合') : t('启用动作组合'),
			content: record.enabled
				? t('确定停用“{{name}}”吗？', { name: record.name })
				: t('确定启用“{{name}}”吗？', { name: record.name }),
			okText: record.enabled ? t('停用') : t('启用'),
			cancelText: t('取消'),
			onOk: () => {
				setRecords((current) =>
					current.map((item) =>
				...
				void message.success(
					record.enabled ? t('动作组合已停用') : t('动作组合已启用'),
				)
			},
		})
	const deleteCombination = (record: ActionCombination) =>
		modal.confirm({
			title: t('删除动作组合'),
			content: t('确定删除“{{name}}”（{{code}}）吗？删除后无法撤销。', {
				name: record.name,
				code: record.code,
			}),
			okText: t('删除'),
	...
	const columns: TableColumnsType<ActionCombination> = [
		...
		},
		{
			...
			width: 104,
			render: (scene: string) => (
				<Tag className={styles.sceneTag}>{t(scene)}</Tag>
			),
		},
		{
			title: t('AGV 类型'),
		...
		// 分支工作流的节点列表仅用于摘要，具体执行路径由编辑器内的连线决定。
		{
			...
			render: (_value, record) => (
				<Flex vertical gap={5}>
					<Flex align="center" gap={5} className={styles.sequence}>
						{record.steps.slice(0, 3).map((step, index) => (
							<span className={styles.sequenceItem} key={index}>
								{index > 0 && (
				...
				<Flex align="center" gap={4}>
					...
					<Dropdown
						...
						menu={{
							...
							onClick: ({ key }) => {
								if (key === 'status') changeStatus(record)
								if (key === 'delete') deleteCombination(record)
							},
						}}
					>
						<Button
	...
	return (
		<Flex vertical gap={18} className={styles.page}>
					...
					<Row gutter={[24, 0]}>
						<Col xs={24} md={12} xl={6}>
							...
							>
								<Input
									allowClear
									placeholder={t('请输入组合名称或编码')}
									prefix={<Search size={14} />}
								/>
							</Form.Item>
						</Col>
						<Col xs={24} md={12} xl={6}>
							<Form.Item label={t('业务场景')} name="scene">
								<Select
									allowClear
									placeholder={t('全部场景')}
						...
						<Col xs={24} md={12} xl={6}>
							<Form.Item label={t('AGV 类型')} name="robotType">
								<Select
									allowClear
									placeholder={t('全部类型')}
									options={robotTypeOptions.map((option) => ({
										...option,
										label: t(option.label),
									}))}
								/>
						...
						<Col xs={24} md={12} xl={6}>
							<Form.Item label={t('启用状态')} name="enabled">
								<Select
									allowClear
									placeholder={t('全部状态')}
									options={[
										{ label: t('已启用'), value: true },
										{ label: t('已停用'), value: false },
									]}
								/>
							</Form.Item>
						</Col>
