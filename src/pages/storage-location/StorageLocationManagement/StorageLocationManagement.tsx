...
import {
	getStorageStation,
	storageLocationData,
} from '@/features/storage-location/storageLocation.model'
import type {
	StorageLocation,
	StorageLocationValues,
} from '@/features/storage-location/storageLocation.types'
...
export default function StorageLocationManagement() {
	...
	const [records, setRecords] = useState<StorageLocation[]>(() =>
		structuredClone(storageLocationData.locations),
	)
	const [selectedKey, setSelectedKey] = useState(
		storageLocationData.locations[0]?.id ?? 'new',
	)
	...
	const visibleRecords = records.filter((record) => {
		...
		return (
			(!stationFilter || stationFilter === record.stationId) &&
			`${record.name} ${station?.name ?? ''} ${point?.name ?? ''}`
				.toLocaleLowerCase()
				.includes(query)
		)
	})
	...
	const confirmDiscard = (action: () => void) => {
		...
		modal.confirm({
			title: '放弃未保存的修改？',
			content: '当前库位参数尚未保存，继续操作将丢弃这些修改。',
			okText: '放弃修改',
			cancelText: '继续编辑',
			onOk: action,
		})
	...
	const saveLocation = (values: StorageLocationValues) => {
		...
		setPage(
			activeRecord
				? Math.floor(
						records.findIndex((record) => record.id === saved.id) /
							PAGE_SIZE,
					) + 1
				: 1,
		)
	...
	const treeData: TreeDataNode[] = [
		{
			...
			children: records.map((record) => ({
				key: record.id,
				title: <span title={record.name}>{record.name}</span>,
				isLeaf: true,
			})),
		},
		{
	...
	const columns: TableColumnsType<StorageLocation> = [
		...
		},
		{
			...
			render: (_value, record) => (
				<Flex vertical gap={3}>
					<span>{getStorageStation(record.stationId)?.name}</span>
					<Typography.Text type="secondary" className={styles.hint}>
						{
							getStorageStation(record.stationId)?.points.find(
								(point) => point.id === record.pointId,
							)?.name
						}
					</Typography.Text>
				</Flex>
		...
		},
		{
			...
			render: (_value, record) => (
				<Flex vertical gap={4}>
					<Space size={8}>
						<Tag className={styles.layerTag}>
							第 {record.layer} 层
						</Tag>
						<span>最大承重 {record.maxLoad} kg</span>
					</Space>
					<Typography.Text type="secondary" className={styles.hint}>
						深度 {record.depth} mm · 高度 {record.height} mm
					</Typography.Text>
				</Flex>
			),
		},
	...
	return (
		<Flex vertical gap={12} className={styles.page}>
			<Flex gap={16} align="start" className={styles.workspace}>
				...
				>
					<Tree.DirectoryTree
						...
						treeData={treeData}
						onSelect={(keys) => {
							if (keys[0]) selectLocation(String(keys[0]))
						}}
					/>
					<div className={styles.navigationNote}>
						选择库位维护参数，或进入总览查看全部配置。
					</div>
				</Card>
					...
					<div ref={editorRef} className={styles.editorAnchor}>
						{selectedKey !== 'overview' && (
							<Card
								...
								extra={
									<Tag color="blue">
										{activeRecord ? '修改库位' : '新增库位'}
									</Tag>
								}
							>
								<StorageLocationEditor
									key={`${selectedKey}:${editorRevision}`}
									record={activeRecord}
									records={records}
					...
					>
						...
						>
							<Space size={10}>
								...
								>
									共 {records.length} 个库位
								</Typography.Text>
							</Space>
							<Button
								type="primary"
								icon={<Plus size={15} />}
								onClick={() => selectLocation('new')}
							>
								新增库位
							</Button>
						</Flex>
						<Flex gap={10} wrap className={styles.filters}>
							...
							<Select
								...
								placeholder="全部站点"
								allowClear
								className={styles.stationFilter}
								value={stationFilter}
								options={storageLocationData.stations.map(
									(station) => ({
										label: station.name,
										value: station.id,
									}),
