...
function closeOthers(
	tabs: TabEntry[],
	keepKey: string,
): { tabs: TabEntry[]; activeTabKey: string } {
	const next = tabs.filter(
		(tab) => tab.affix || !tab.closable || tab.key === keepKey,
	)
	...
	reducers: {
		...
		tabSynced(state, action: PayloadAction<TabSyncPayload>) {
			...
			if (cacheable) {
				const cachedNonAffix = state.tabs
					.filter(
						(tab) => tab.cached && !tab.affix && tab.key !== tabKey,
					)
					.sort((a, b) => a.activatedSeq - b.activatedSeq)
				for (const victim of cachedNonAffix.slice(
					0,
					Math.max(
						0,
						cachedNonAffix.length - (PAGE_CACHE_MAX_ENTRIES - 1),
					),
				)) {
					victim.cached = false
				}
		...
		tabClosed(state, action: PayloadAction<string>) {
			...
			if (wasActive) {
				...
				state.activeTabKey =
					right?.key ?? left?.key ?? state.tabs[0]?.key ?? null
			}
		},
		/** 关闭其他页签（affix 永不受影响） */
		...
		leftTabsClosed(state, action: PayloadAction<string>) {
			...
			const removedActive = state.tabs
				.slice(0, index)
				.some((tab) => tab.key === state.activeTabKey)
			state.tabs = state.tabs.filter(
				(tab, i) => i >= index || !tab.closable || tab.affix,
			)
		...
		tabMoved(
			...
			const from = state.tabs.findIndex(
				(tab) => tab.key === action.payload.fromKey,
			)
			const to = state.tabs.findIndex(
				(tab) => tab.key === action.payload.toKey,
			)
