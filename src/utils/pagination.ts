...
/** 归一化后的分页参数：page 从 1 开始，pageSize 在 [1, PAGE_SIZE_MAX] 内 */
 ...
 * - pageSize 为正整数时截断到 PAGE_SIZE_MAX，非法或小于 1 时回退 PAGE_SIZE_DEFAULT。
 */
export function normalizePagination(input: {
  ...
  const pageSize = isPositiveInteger(input.pageSize)
    ? Math.min(input.pageSize, PAGE_SIZE_MAX)
    : PAGE_SIZE_DEFAULT
