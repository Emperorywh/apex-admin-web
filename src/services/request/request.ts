...
function fromAxiosError(error: AxiosError): ApiError {
	...
	if (typeof response.data === 'string')
		dataText = response.data.slice(0, 200)
	return {
		isApiError: true,
...
let requestHealth: RequestHealth = {
	lastStatus: null,
	lastCheckedAt: 0,
...
export function subscribeRequestHealth(
	listener: (health: RequestHealth) => void,
): () => void {
...
async function handleRequestError(error: AxiosError): Promise<unknown> {
	const config = error.config as
		(InternalAxiosRequestConfig & AxiosRequestConfig) | undefined
	...
	if (!canceled)
		recordHealth(
			responded && error.code !== 'APEX_HTML_RESPONSE' && !unreachable,
		)
...
async function refreshAccessToken(): Promise<string> {
	...
	refreshPromise = axios
		...
		.then((response) => {
			const token = (response.data as { accessToken?: string } | null)
				?.accessToken
