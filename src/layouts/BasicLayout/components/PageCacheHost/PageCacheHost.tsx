...
export function PageCacheHost({
	...
	if (prevTabs !== tabs || prevActiveKey !== activeTabKey) {
		...
		setDyingTabs((prev) => {
			...
			if (
				removedActiveTab !== undefined &&
				!next.some((tab) => tab.key === removedActiveTab.key)
			) {
		...
		for (const tab of dyingTabs) {
			...
			timers.set(
				...
				window.setTimeout(() => {
					...
					setDyingTabs((prev) =>
						prev.filter((item) => item.key !== tab.key),
					)
				}, CLOSE_FALLBACK_MS),
			)
		}
		...
		<>
			{tabs
				...
				.map((tab) => (
					<Activity
						key={tab.key}
						mode={tab.key === activeTabKey ? 'visible' : 'hidden'}
					>
						<PageLayer>
							<CachedPage
								tab={tab}
			...
			{dyingShown.map((tab) => (
				<Activity key={tab.key} mode="visible">
					<PageLayer
						closing
						onCloseAnimationEnd={() => settleClosedTab(tab.key)}
					>
			...
			{currentMeta !== null &&
				(currentMeta.noCache === true ||
					currentMeta.hideInTabs === true) && (
					<PageLayer>
						<CurrentPage
