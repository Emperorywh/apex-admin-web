/**
 * 基础布局：工业调度控制台（品牌 + 页签 + 状态 / 工作区 / 底部菜单座）。
 *
...
function resolveLeaf(matches: RouterMatches): ActiveLeaf | null {
	for (let i = matches.length - 1; i >= 0; i -= 1) {
		...
		if (
			match.id === ROUTE_IDS['root'] ||
			match.id === ROUTE_IDS['root-index']
		)
			continue
...
export function BasicLayout() {
	...
	const activeTab = useMemo(
		() =>
			tabsState.tabs.find((tab) => tab.key === tabsState.activeTabKey) ??
			null,
		[tabsState],
	)
	...
	const activeTitle = activeTab
		? findRouteMeta(activeTab.routeId)?.title
		: leaf?.meta.title
	useEffect(() => {
		if (activeTitle) {
