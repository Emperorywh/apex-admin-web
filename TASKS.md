...
> 48 项任务及业务验收范围不变。39 项历史实现保留，当前 P21 代码保留；调度为「未完成任务优先」：历史任务的验收复核顺延至全部页面实现收口后逐项补做（每轮一项 audit），复核通过前不勾选；复选框只表示最终验收全部通过，不能用来判断实施队列。
...
启动先补交付未确认推送的提交，然后恢复未完成任务（currentTaskId 及剩余批次）的开发；历史任务的验收复核顺延至全部页面实现收口后逐项补做（每轮一项 audit），复用已有代码和证据，不整页重写。开发依赖按第 3 节口径以「代码/契约已合并」判定，不要求前置任务先通过复核。实现、门禁或推送受阻不得挑选后续任务；本任务可做部分完成后保存检查点退出。已放行但仍待现场/跨页验收的任务保留未勾选，由指定责任任务提供联验依据，再逐任务执行 acceptance 更新自身记录；进入 V01 前先完成顺延的历史复核并清空前项待验收队列。V01 未通过不能声称整体完成。
