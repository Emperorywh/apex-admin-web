...
后端（`C:\code\apex-admin`，FastAPI）默认代理到 `http://localhost:8000`，可用 `APEX_DEV_PROXY_TARGET` 覆盖；**DEV 下后端不可达时自动切换内存演示后端**（任意已有用户名 + ≥12 位密码即可登录，`admin` 拥有全部权限）。
...
- `src/pages` — 页面入口（登录 / 运营总览 / 个人中心 / 系统管理 / 错误页）
