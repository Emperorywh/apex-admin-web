...
> 版本：v1.6 · 日期：2026-08-18 · 状态：已确认（两阶段实施完成 + v1.3 导航走查回归修复，见 §16 修订记录）
>
> v1.6 修订记录（2026-08-18）：随主规格 v1.12 移除演示模式——Header「演示模式标记」与登录页 demo 账号提示卡从头部/登录页元素构成中移除（§2.2/§5.2/§9 相关条文失效）；菜单管理图标列保留（`icon` 为后端可选字段，缺省呈现占位）。
>
...
- 头部元素构成：折叠触发器 / 面包屑 / 全屏 / 语言 / 主题快捷切换 / 用户菜单 / 设置入口（演示模式标记已随主规格 v1.12 移除）。
...
7. 菜单管理演示页**新增图标列**（本规格唯一列结构例外，§7 同步豁免）：`icon` 为后端菜单数据可选字段（值为 `local:` 图标名），主规格 §14.1 `MenuItem` 契约不变；后端数据无该字段时图标列呈现占位；渲染统一经 AppIcon。
...
- 工具图标按钮：统一 36px 圆形 hover 容器（slash 风）；元素构成与顺序不变（演示模式 Badge 已随主规格 v1.12 移除）。
...
- 新增共享骨架组件（`src/components/` 下，命名如 `PageCard`，实施定稿），统一标题区/搜索区/内容区节奏；列表页逐页套用：用户/角色/菜单管理、个人中心。
- 列结构、查询字段、按钮级权限、交互零变更；表格仅经 §4.2 token 与骨架间距变化。**唯一例外**（§5.7）：菜单管理页新增图标列（后端可选 `icon` 字段驱动，`MenuItem` 契约不变，缺省数据呈现占位）。
...
- Dashboard 时间序列数据由真实后端 `/dashboard/overview` 提供（演示模式已随主规格 v1.12 移除）。
- 信息架构不新增业务模块（无新页面、无新路由）。

## 9. 登录页与全局反馈页（已定）

- 登录页：左右分栏结构与表单行为零变更；继承新 token（圆角/字体/14px/色板）与卡片规范；加 **motion 淡入/上滑入场**（登录页非缓存路由，无 Activity 冲突）。
