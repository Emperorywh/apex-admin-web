...
| G05 | `uploadSystemVersion` 等上传在文档表现为 `application/json` 内 binary 字段 | P08/P09/P25/P27（P23 若有可达上传同样核实） | 部分确认（P08 实证）；其余上传随各自页面核实 | **2026-09-19 P08 实证：uploadVehicleAlarmCodeFile 文档声明 `application/json`+binary，真实后端只接受 multipart/form-data、字段名 `file`**（curl multipart `code=200`；文档形态未采用）；服务层按旧实现 multipart 适配。P09/P25/P27 的上传仍需逐个实证，不沿用本结论 | 2026-09-19（P08 curl 实证 + 浏览器整页上传） |
