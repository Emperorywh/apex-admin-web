/**
 * P01 audit 复核轮（run-0142d6d6，基线 bbe05cb）：登录链路真实复测（无业务写副作用）。
 *
 * 与上轮 audit（audit-20260920-p01-44fc5e6）的差异：基线后 definitions.tsx 中
 * /dashboard 节点解除 pending（P34 交付），登录落点规则兑现「有权限即落 /dashboard」。
 * 因此本轮在保留上轮 5 项登录链路复测（错误凭据/正确凭据 MD5/品牌列表/登录背景
 * 文件位回退条件/登出清理）之外，新增两项落点条件核查：
 *   6. 登录返回权限码包含 dashboard-realtime:view（/dashboard 同码）——落点前提；
 *   7. 带令牌 GET 只读（auth/user 分页）验证 token 真实可用（同 T00 audit 口径）。
 *
