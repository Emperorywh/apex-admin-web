/**
 * P01 audit 复核轮：登录链路真实复测（无业务写副作用）。
 *
 * 与 T00 audit 轮只读复测（同日、代码同基线）的分工：T00 轮覆盖「正确凭据登录 +
 * 带令牌只读 + 无令牌对照 + 登出」；本脚本补 P01 任务卡专项验收的关键缺口——
 * 「错误凭据按真实结果反馈（业务码 1000010）」，并复核 P01 直接消费的品牌通道
 * （systemLogos 列表与 loginBackground 文件位在当前环境的真实表现，即登录页
 * 品牌背景回退的触发条件）。
 *
 * 凭据仅从本机既有安全来源 `.env.local`（APEX_DEV_LEGACY_TARGET /
 * APEX_TEST_USERNAME / APEX_TEST_PASSWORD）读取；本脚本与输出均不含密码与 token。
 *
 * 用法：node run-p01-audit-readonly.mjs [baseUrl]
...
// 密码取真实密码 + 后缀缀（保证 MD5 值不同且几乎不可能碰中），用户名真实存在，
