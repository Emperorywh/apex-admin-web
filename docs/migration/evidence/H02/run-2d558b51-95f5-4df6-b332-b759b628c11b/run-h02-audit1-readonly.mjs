/**
 * H02 audit 第一次复核真实只读复测脚本（run run-2d558b51，代码基线 76319c4）。
 *
 * H02 地图编辑暂缓说明页无业务接口，本脚本验证与页面行为相关的真实前提：
 * - dev 同源代理链路可达（浏览器验收同通道）：getHardwareInfo 无令牌；
 * - 代理与直连后端一致性对照；
 * - 真实登录（MD5+Bearer）读取 activated/权限码，断言：
 *   ① map-edit:view 在权限集合（/map-through/map-nest-modify 守卫与菜单
 *     显隐前提，原权限 MAP_EDIT_VIEW 保留——H02 交付核对两侧常量一致）；
...
// ---- 3. 真实登录读取 H02 两项前提（经 dev 代理，同浏览器通道） ----
...
if (!username || !password) {
  ...
  try {
    ...
    if (ok) {
      ...
      const hasMapEdit = perms.includes('map-edit:view')
      record(
        'H02 守卫前提：map-edit:view 在权限集合',
        hasMapEdit,
        hasMapEdit
          ? '包含 → /map-through/map-nest-modify 菜单可见+直访守卫放行（原权限 MAP_EDIT_VIEW 保留核对成立）'
      ...
      record(
        ...
        hasDashboard,
        hasDashboard
          ? '包含 → resolveLandingPath 落 /dashboard；带 redirect=%2Fmap-through%2Fmap-nest-modify 登录应被拒并回落 /dashboard（deferred 排除链路）'
...
const summary = {
  runId: 'run-2d558b51-95f5-4df6-b332-b759b628c11b',
  taskId: 'H02',
  purpose: 'audit（第一次复核）',
  codeCommit: '76319c4e6fc2083fa7100d7311ce79dad36656bd',
...
writeFileSync(join(here, 'h02-audit1-readonly-result.json'), JSON.stringify(summary, null, 2))
console.log(`\n${summary.passCount}/${summary.totalCount} PASS；结果写入 h02-audit1-readonly-result.json`)
