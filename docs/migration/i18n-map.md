...
| P24 动作分组 | mission-cluster/action-control/agv-action-group | **agvActionGroup**（页面私有命名空间；已交付四语言分片同构：en 旧真译 18 键逐条沿用（Query AGV Action Groups/Action Group Name/Group Actions/Add AGV Action Group/Edit AGV Action Group/Please enter (select) action group name·group action/Action group added-edited-deleted successfully/Error querying-adding-editing-deleting action group 等全量）+ 删除确认标题（Delete AGV Action Group）与影响说明/错误 msg 插值（Error … action group: {{msg}}）/新增分组（Add Group，旧 en 缺失补译）按旧真译风格补译；繁日韩旧资源对本页 key 均为简中复制（非有效译文），按 B1 基线补译：繁中 動作分組（与 menu 分片对齐）/動作分組名稱/儲存 台湾惯用语，日文 アクショングループ（与 menu 分片对齐）/グループアクション/車両アクショングループを追加·編集，韩文 액션 그룹（与 menu 分片对齐）/그룹 액션/차량 액션 그룹 추가·수정；失效动作识别（Action #{{id}} (unavailable)/{{count}} action(s)…no longer available）与 P11 句式对齐三语补译；公共词与 P22/P23 分片对齐（照会/조회、確認/확인、取消=キャンセル·취소），登记本行） | features/action-control + pages/action-control/AGVActionGroup + services/action/agv-action-group-manage |
| P20 任务工艺 | mission-cluster/mission-create | **orderTemplate**（页面私有命名空间；已交付四语言分片同构：en 旧真译 58 键逐条沿用（Search Tasks/Create Task/Task Name/Task ID/Vehicle (Group)/Copy/Copy Task/Edit Task/Template Name/Assign Vehicle (Group)/Subtask {index}/Map Name/Station Name/Action Type/Description/Block Type/Action Name/Action Value/Add Subtask (Action)(Param)/Subtasks cannot be empty/Task created-updated-deleted successfully/Error querying cross-map nodes 等）+样板升级补译：删除确认标题 Delete Task/影响说明/错误 msg 插值（Error creating-updating-deleting task: {{msg}}，旧「文案+message 拼接」形态修正）/查询 Query；旧键「确定删除当前任务?」随 confirmCommand 升级退役、「查询任务组合列表出错」随 Apex 内建错误态退役（Apex 公共 locale 包承接）；繁日韩旧资源对本页 key 均为简中复制（非有效译文），按 B1 基线补译：繁中 任務名稱/建立任務/範本名稱/複製任務/新增子任務（任务/模板术语与 orderRecord 分片对齐），日文 タスク名/タスク作成/テンプレート名/アクション種別/説明/ブロック種別/パラメータ/サブタスク（与 P23 agvAction 分片对齐），韩文 작업명/작업 생성/템플릿 이름/액션 유형/설명/블록 유형/파라미터/하위 작업（与 P23 agvAction 分片对齐）；地图/站点术语三语与 crossMap 分片对齐（地圖/地図/지도，站點/ステーション/스테이션）；公共词与 P22/P23 分片对齐） | features/mission-cluster/components/MissionTemplateModal + pages/mission-cluster/MissionCreate + services/order-template/order-template-manage |
...
| P20 任务工艺 | mission-create | orderTemplate | orderTemplate |
...
| P35 任务统计报表 | dashboard-task | **report-task**（已交付四语言分片 57 key 同构，替代冻结时预估名 taskReport：KPI 标签/图卡标题/桶标签/aria en 旧真译逐条沿用（含 4 条 hint 长译文），繁日韩旧有标题/aria 真译沿用、缺失词条（口径说明/快捷预设/明细列头/暂无数据）按 B1 基线补译并与 P36/P37 分片对齐——查詢/検索/조회、暫無資料/データがありません/データ 없음；旧 {days} 单花括号插值改 {{days}}；口径说明类 key 一律完整简中文案（短 key 在 zh-CN 显示 key 本体且插值无法应用——P33 纪律 P35 轮再次实测发现并修复）；明细列头/口径说明为本任务新增文案；nsSeparator=false 下 t() 不带「ns:」前缀） | 重写 pages/analyze-visual/TaskStatisticsReport + features/analyze-visual/task-statistics + services/report-task |
| P36 故障告警 | dashboard-fault | **report-fault**（已交付四语言分片 62 key 同构：表格列/筛选/级别/状态 en 旧真译逐条沿用，旧「AGV」列名按术语表收敛为「车辆」；繁日韩旧资源为简体复制或缺失按 B1 基线补译并与 orderRecord/dashboard 分片对齐——查詢/検索/조회、暫無資料/データなし/데이터 없음、開始時刻/開始時刻/시작 시각；6 条口径说明+新增详情字段按旧真译风格补译；告警描述译文走服务端译文回退链当前语言→zh_CN→en_US→原文，客户端不臆造；「查看全部告警」key 入 dashboard 分片（P34 入口共用）；nsSeparator=false 下 t() 不带「ns:」前缀） | 重写 pages/analyze-visual/FaultAlert + features/analyze-visual/fault-alert + services/report-fault |
| P37 车辆状态统计 | vehicle-status | **report-vehicle-state**（已交付四语言分片 42 key 同构：KPI 标签/状态枚举 13 词/表格列/工具栏 en 旧真译逐条沿用（含 5 条「·计算方式」长文案），旧「AGV」按术语表收敛为「车辆/車両/차량」；繁日韩旧资源缺失按 B1 基线补译——執行作業/作業実行中/작업 실행 중、佔該車總時長/車両合計時間に占める割合/차량 총 시간 대비 비율；插值占位一律 {{name}} 双花括号（{name} 单花括号在简中 key 即文案下原样显示，本轮实测教训）；nsSeparator=false 下 t() 不带「ns:」前缀） | 重写 pages/analyze-visual/VehicleStatus + features/analyze-visual/vehicle-status + services/report-vehicle-state |
