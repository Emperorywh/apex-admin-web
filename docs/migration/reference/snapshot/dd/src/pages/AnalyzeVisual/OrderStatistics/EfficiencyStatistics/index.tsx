...
import { orderEfficiencyStatistics, getSimpleVehicles } from "@/api";
...
const formatSecondsToTime = (seconds: number, t: (id: string) => string): string => {
    if (seconds <= 0) return t("0秒");
    const hours = Math.floor(seconds / 3600);
    const minutes = Math.floor((seconds % 3600) / 60);
    const secs = Math.round(seconds % 60);
    let result = "";
    if (hours > 0) result += `${hours}${t("时")}`;
    if (minutes > 0) result += `${minutes}${t("分")}`;
    if (secs > 0 || result === "") result += `${secs}${t("秒")}`;
    return result;
};

const EFFICIENCY_COLORS = ["#5B8FF9", "#5AD8A6", "#F6BD16"];
...
const getThemeColors = (localTheme?: string) => {
    ...
    return {
        ...
        /** 坐标轴名称颜色 */
        nameText: isDark ? "#9ca3af" : "#6b7280",
    };
};

const EfficiencyBar = memo((props: { height: number; data: EfficiencyDataItem[]; title: string; t: (id: string) => string }) => {
    ...
    useEffect(() => {
        ...
        if (data?.length) {
            for (const d of data) {
                totalTime += d.orderAverageTime || 0;
                execTime += d.orderAverageExecutionTime || 0;
                waitTime += d.orderAverageWaitTime || 0;
            }
        }

        const categories = [t("平均总时间"), t("平均执行时间"), t("平均等待时间")];
        ...
        const series: BarSeriesOption[] = [{
            ...
            itemStyle: {
                ...
                color: (params) => EFFICIENCY_COLORS[params.dataIndex % EFFICIENCY_COLORS.length]
            },
            data: [
                +totalTime.toFixed(2),
                +execTime.toFixed(2),
            ...
            label: {
                ...
                formatter: (p) => {
                    ...
                    return val ? formatSecondsToTime(val, t) : "";
                },
                fontSize: 12,
        ...
        const option: OptionType = {
            ...
            tooltip: {
                ...
                valueFormatter: (value) => formatSecondsToTime(value as number, t)
            },
...
/* 国际化翻译方法 */ const { t } = useI18n();
    ...
    const [params, setParams] = useState<OrderStatisticsParams>({
        startTime: "",
        endTime: ""
    });

    const [efficiencyData, setEfficiencyData] = useState<EfficiencyDataItem[]>([]);
    ...
    const onStartChange: DatePickerProps["onChange"] = (_date, dateString) => {
        setParams(prev => ({ ...prev, startTime: dateString as string || "" }));
    };

    const onEndChange: DatePickerProps["onChange"] = (_date, dateString) => {
        setParams(prev => ({ ...prev, endTime: dateString as string || "" }));
    };

    const onOrderTypesChange = (values: string[]) => {
        setParams(prev => ({ ...prev, orderTypes: values }));
    };

    const onOrderStatesChange = (values: string[]) => {
        setParams(prev => ({ ...prev, orderStates: values }));
    };

    const onVehicleKeysChange = (values: string[]) => {
        setParams(prev => ({ ...prev, vehicleKeys: values }));
    };

    const handleQuery = useCallback(() => {
        setLoading(true);
        orderEfficiencyStatistics(params).then(res => {
            if (res.code === 200 && res.message === "success") {
                setEfficiencyData(res?.data || []);
            } else {
                message.warning(t("查询任务效率统计出错：") + res?.message);
            }
        }).catch(err => {
            if (err) {
                message.error(t("查询任务效率统计出错：") + err?.message);
            }
        }).finally(() => {
            setLoading(false);
        });
