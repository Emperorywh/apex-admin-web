        ...
        },
        {
            ...
            render: (_, record) => (
                <Space>
                     ...
                     * 入口已控权，弹窗 RoleModal 内提交按钮无需重复控权（§7.7）
                     */}
                    {hasPerm(PERM_BUTTON.AUTH_ROLE_UPDATE) && (
                        <Button type="primary" onClick={() => handleEdit(record)}>
                            {t("编辑")}
                        </Button>
                    )}
                    {/*
                     * 分配权限：无 auth:role:assign-permission 权限隐藏
                     * 入口已控权，弹窗 PermissionModal 内提交无需重复控权（§7.7）
                     */}
                    {hasPerm(PERM_BUTTON.AUTH_ROLE_ASSIGN_PERMISSION) && (
                        <Button onClick={() => handleAssignPermission(record)}>
                            {t("分配权限")}
                        </Button>
                    )}
                    {/* 删除角色：无 auth:role:delete 权限隐藏 */}
                    {hasPerm(PERM_BUTTON.AUTH_ROLE_DELETE) && (
                        <Popconfirm
                            title={t("删除角色")}
    ...
    return (
        <div className={styles.role_management}>
            <div className={styles.role_header}>
                ...
                <Space>
                    {/*
                     * 新增角色：无 auth:role:add 权限条件渲染隐藏
                     * 动作触发型按钮（工具栏独立按钮，§7.1）
                     */}
                    {hasPerm(PERM_BUTTON.AUTH_ROLE_ADD) && (
