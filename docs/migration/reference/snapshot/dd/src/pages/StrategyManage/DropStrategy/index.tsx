...
import { Button, message, PaginationProps, Popconfirm, Space, Table, TableProps, Input } from "antd";
import { dropStrategyType, PageParams } from "../type";
...
export default () => {
    ...
    const strategyColumns: TableProps<dropStrategyType>["columns"] = [
        {
            title: t("策略名称"),
        ...
        },
        {
            ...
            fixed: "right",
            width: 200,
            render: (_, record: dropStrategyType) => (
                <Space size="middle">
    ...
    const [strategyData, setStrategyData] = useState<dropStrategyType[]>();

    const [showEditModal, setShowWditModal] = useState<boolean>(false)
    const [currentStrategyData, setCurrentStrategyData] = useState<dropStrategyType | null>(null)
    ...
    // 编辑策略
    const handleEditStrategy = (record: dropStrategyType | null) => {
        // console.log({record})
        setCurrentStrategyData(record)
        setShowWditModal(true)
    }
    // 删除策略
    const handleDeleteStrategy = async (record: dropStrategyType) => {
        const { id } = record
    ...
    const onTableChange: TableProps<dropStrategyType>["onChange"] = (pagination) => {
        ...
        <div style={{ padding: 20 }}>
            ...
            >
                <Search
                    ...
                    enterButton
                />
                <Button type='primary' onClick={() => handleEditStrategy(null)}>{t("新增放货策略")}</Button>
            </Space>
            <Table<dropStrategyType>
