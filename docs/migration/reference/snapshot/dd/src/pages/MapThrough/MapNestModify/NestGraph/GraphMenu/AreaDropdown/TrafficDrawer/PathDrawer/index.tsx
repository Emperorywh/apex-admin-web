...
/* 国际化翻译方法 */ const { t } = useI18n();
    ...
    const { stage, currentArea, openLineDrawer, trafficGroups, setOpenLineDrawer, setTrafficGroups } = props;
    ...
    const onCheckboxChange: GetProp<typeof Checkbox.Group, "onChange"> = (checkedValues) => {
        ...
        if (stage && currentArea) {
            const prev = trafficGroups.find(a => a.id === currentArea);
    ...
    useEffect(() => {
        ...
        const pathOpts: CheckboxOptionType<string>[] = elements.map(shape => ({
            label: shape.attrs?.data?.name + `【${shape.attrs?.enableSelect === "node" ? t("节点") : t("路径")}】`,
            value: shape.attrs?.id
        }))
        setElementList(pathOpts);
    }, [openLineDrawer])

    useEffect(() => {
        if (!trafficGroups?.length || !currentArea) return;
        const target = trafficGroups.find(area => area.id === currentArea);
        console.log("拿到的目标", target)
        if (!target) return;
        setCheckedElements([...target?.edgeIds, ...target.nodeIds]);
    }, [trafficGroups, currentArea])
