...
/* 国际化翻译方法 */ const { t } = useI18n();
    ...
    const onCXChange: InputNumberProps["onChange"] = (value) => {
        if (!selectShapes?.length || typeof value !== "number") return;
        selectShapes.forEach(shape => {
            // 改变画布中的控制点A的坐标
            const stage = shape.getStage();
            const controlPoint = stage?.findOne("#controlPointA");
            ...
            const { attrs: { points: [ssx, ssy, csx, csy, dsx, dsy, esx, esy] } } = controlLine as Konva.Shape;
            controlLine?.setAttr("points", [ssx, ssy, value, csy, dsx, dsy, esx, esy]);
            /**
             * 反向边联动判定（基于变更前的 attrs.data）：
             *   - 若当前边与反向边轨迹重合，则同步把 cx 写入反向边的 dx；
             *   - 否则反向边保持原状，避免破坏非重合状态下的独立编辑。
             */
            ...
            // 贝塞尔曲线
            const points: BezierPoints = [sx, -sy, value, -cy, dx, -dy, ex, -ey];
            // 计算贝塞尔曲线的箭头坐标
            ...
            /**
             * 反向边镜像写入：cx 改变 → 反向边的 dx 同步为 value，
